        ...
        return new Promise((innerResolve, innerReject) => {
          Keep.findOne({ allNames: result.matches[0].value }, (err, keep) => {
            let discordResponse = '```css\r\n' +
              `[${keep.allegiance}]${keep.name} #${keep.level}\r\n` +
              `coords: (${keep.location.x}, ${keep.location.y}) {shield: ${keep.peaceshield ? 'UP' : 'DOWN'}} {power: ${parseInt(keep.power)}}\r\n` +
              `{names: '${keep.allNames.join(' ')}'}\r\n` +
              `{allegiances: '${keep.allAllegiances.join(' ')}'}\r\n` +
              '```';
